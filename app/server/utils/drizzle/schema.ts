import { pgTable, text, index, foreignKey, bigint, integer, smallint, timestamp, boolean, uniqueIndex, varchar, uuid, date, pgEnum } from 'drizzle-orm/pg-core'
...
export const championBan = pgTable('champion_ban', {
	...
	championId: smallint('champion_id').notNull(),
}, table => [
	index('index_champion_ban_gameid').using('btree', table.gameId.asc().nullsLast().op('int8_ops')),
	index('index_champion_ban_teamid').using('btree', table.teamId.asc().nullsLast().op('int4_ops')),
	foreignKey({
...
export const game = pgTable('game', {
	...
	gameStart: timestamp('game_start', { precision: 6, withTimezone: true, mode: 'string' }).notNull(),
})
...
export const gameMatchupRelation = pgTable('game_matchup_relation', {
	...
	index('index_game_matchup_relation_gameid').using('btree', table.gameId.asc().nullsLast().op('int8_ops')),
	index('index_game_matchup_relation_matchupid').using('btree', table.matchupId.asc().nullsLast().op('int4_ops')),
	foreignKey({
		columns: [table.gameId],
		foreignColumns: [game.gameId],
...
export const gamePlayerStats = pgTable('game_player_stats', {
	...
	index('index_game_player_stats_gameid').using('btree', table.gameId.asc().nullsLast().op('int8_ops')),
	index('index_game_player_stats_puuid').using('btree', table.puuid.asc().nullsLast().op('text_ops')),
	foreignKey({
		columns: [table.gameId],
		foreignColumns: [game.gameId],
...
export const gameTeamStats = pgTable('game_team_stats', {
	...
	index('index_game_team_stats_gameid').using('btree', table.gameId.asc().nullsLast().op('int8_ops')),
	index('index_game_team_stats_teamid').using('btree', table.teamId.asc().nullsLast().op('int4_ops')),
	uniqueIndex('unique_game_team').using('btree', table.gameId.asc().nullsLast().op('int4_ops'), table.teamId.asc().nullsLast().op('int4_ops')),
	foreignKey({
		columns: [table.gameId],
		foreignColumns: [game.gameId],
...
export const matchup = pgTable('matchup', {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	matchupId: bigint('matchup_id', { mode: 'number' }).generatedByDefaultAsIdentity({ name: 'matchup_matchup_id_seq', startWith: 1, increment: 1, minValue: 1, maxValue: '9223372036854775807', cache: 1 }),
	createdAt: timestamp('created_at', { precision: 6, withTimezone: true, mode: 'string' }).default(sql`CURRENT_TIMESTAMP`).notNull(),
	...
	index('index_matchup_tournamentid_team2id').using('btree', table.tournamentId.asc().nullsLast().op('int4_ops'), table.team2Id.asc().nullsLast().op('int4_ops')),
	foreignKey({
...
export const player = pgTable('player', {
	puuid: text().notNull(),
	createdAt: timestamp('created_at', { precision: 6, withTimezone: true, mode: 'string' }).default(sql`CURRENT_TIMESTAMP`).notNull(),
	lastUpdated: timestamp('last_updated', { precision: 6, withTimezone: true, mode: 'string' }).default(sql`CURRENT_TIMESTAMP`).notNull(),
...
export const team = pgTable('team', {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	teamId: bigint('team_id', { mode: 'number' }).generatedByDefaultAsIdentity({ name: 'team_team_id_seq', startWith: 1, increment: 1, minValue: 1, maxValue: '9223372036854775807', cache: 1 }),
	createdAt: timestamp('created_at', { precision: 6, withTimezone: true, mode: 'string' }).default(sql`CURRENT_TIMESTAMP`).notNull(),
	tournamentId: integer('tournament_id').notNull(),
	name: text().notNull(),
	shorthand: text(),
...
export const tournament = pgTable('tournament', {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	tournamentId: bigint('tournament_id', { mode: 'number' }).generatedByDefaultAsIdentity({ name: 'tournament_tournament_id_seq', startWith: 1, increment: 1, minValue: 1, maxValue: '9223372036854775807', cache: 1 }),
	createdAt: timestamp('created_at', { precision: 6, withTimezone: true, mode: 'string' }).default(sql`CURRENT_TIMESTAMP`).notNull(),
